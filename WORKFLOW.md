...
## VI. Биллинг и Подписки

Логика в `api/payment.js` и `hooks/useSubscription.ts`.

1.  **Инициализация Оплаты**:
    *   Поддерживаются провайдеры: **ЮKassa** и **Prodamus**.
    *   Конфигурация хранится в БД (`system_settings`) или ENV.
    *   При оплате передается `save_payment_method: true` для рекуррентных списаний.
2.  **Обработка Webhook**:
    *   При успешной оплате обновляется `publicMetadata` пользователя в Clerk.
    *   Устанавливается `plan: 'pro'` и `expiresAt`.
3.  **Автопродление (Cron)**:
...
3.  **Конфигурация Платежей**:
    *   Переключение между ЮKassa и Prodamus "на лету".
    *   Ввод API ключей через UI админки.
