...
1.  **Инициализация**:
    ...
    *   Лимит берется из динамической конфигурации в БД (`system_settings` -> `feature_flags`), управляемой через **Admin Panel** -> **Features**.
2.  **Структура Данных**:
...
62. **Smart Polling (Sync)**:
    *   **Active Mode**: Опрос сервера каждые 15 секунд, пока пользователь взаимодействует с плеером (мышь/клавиатура).
    *   **Idle Mode**: Переход на опрос раз в 5 минут после 60 секунд бездействия.
    *   **Force Sync**: Мгновенное обновление при отправке комментария.
...
*   **Tab: Invite Team (Member)**:
    *   **Механика:** Ссылка с параметром `?invite=true` или ввод email.
...
2.  **Инициализация Оплаты**:
    *   Поддерживаются провайдеры: **ЮKassa** и **Prodamus**. Переключение в **Admin -> Integrations**.
...
2.  **Управление Пользователями (Users Tab)**:
    *   Таблица всех пользователей с поиском.
    *   Ручная выдача статуса PRO (Lifetime или на срок).
    *   Назначение других администраторов (Toggle Admin Role).

3.  **Настройки Системы (Features Tab)**:
    *   Глобальное управление доступностью функций (AI, Drive, Export).
    *   Настройка Feature Flags для Free и Pro тарифов раздельно.
    *   Изменение версии приложения (`useAppVersion`).

4.  **Редактор Тарифов (Integrations Tab)**:
    *   Визуальный Drag & Drop редактор карточек тарифов.
    *   Настройка цен и секретных ключей API платежных систем.
...
1.  **Запуск**: Откройте `https://anotee.com/test` или нажмите "System Tests" в Админке.
2.  **Функции**:
