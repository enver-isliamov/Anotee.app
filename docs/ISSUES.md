...
## Сессия 2026-09-21 (третья): T-151…T-153 — оптимизация сборки

| # | Коммит | Суть |
|---|--------|------|
| T-151 | 6144013 / a450b65 | manualChunks: vendor-react (188 KB), vendor-icons (53), vendor-i18n (59) + правила для aws/hf/genai/vercel |
| T-152 | 6144013 | Выделение тяжёлых зависимостей (правила на будущее — сейчас они только в воркере) |
| T-153 | 6144013 | React.lazy + Suspense для AdminPanel, TestRunner, PublicViewer, Roadmap, Legal, LiveDemo, StaticPages |
| — | a450b65 | Исправлены useless escapes в manualChunks (lint) |

Результат: основной чанк **896 → 365 KB (−59%)**, предупреждение сборки о чанках >500 KB устранено.
Проверки: tsc 0 · verify 0 · build 0 · e2e 30 тестов passed (включая public-view с ленивым PublicViewer и diagnostics с ленивым TestRunner).
Воркер транскрибации (857 KB) создаётся только при запуске транскрибации — первая загрузка его не тянет (проверено по коду: new Worker внутри функций transcribe*).

## Сессия 2026-09-21 (вторая): T-146…T-149 — вёрстка проекта и высоты экранов

| # | Коммит | Суть |
|---|--------|------|
| T-145 | 83fd90b | e2e граничных случаев: пустая форма хранилища, короткий токен, закрытие модалки, страница диагностики |
| T-146 | 1377fea | ProjectView: корень 100dvh + overflow-hidden; шапка safe-top + sticky; сетка плиток items-stretch, xl:grid-cols-5; карточка flex-col h-full; превью aspect-video + object-cover (без искажений) |
| T-147 | 1377fea | e2e: пропорции превью 16:9 (±0.06), пересечение иконки и текста ≤1px², отсутствие горизонтального сдвига |
| T-148 | 1377fea | Скриншоты ProjectView (desktop-06, mobile-07) |
| T-149 | 3931ccb | LiveDemo: h-screen/w-screen → h-[100dvh] w-full (динамическая высота, без горизонтального сдвига) |

Проверки на 3931ccb: tsc 0 · verify 0 (unit 61/61, инварианты 5/5) · e2e 14/14 в последнем прогоне (player/landing/mobile-shell) + 24 ранее.

## Сессия 2026-09-21: T-120…T-142 — оболочка, редизайн хранилища, скролл

| # | Коммит | Суть |
|---|--------|------|
| T-125/T-136 | 92c746a / 8d3fe3b | Регрессия скролла: мои правила html{overflow-x:hidden} и video/svg{height:auto} ломали вертикальную прокрутку — убраны; остались безопасные (только body, только мобильные) |
| T-132/T-135 | 3eb0bae | Мастер в 3 шага удалён, у каждого провайдера своя инструкция и прямая ссылка; ссылки проверены HTTP-запросами |
| T-129/T-130 | 3eb0bae | cf_probe валидирует Account-токен через /r2/buckets (вместо недоступного permission_groups); fallback на документированные ID групп прав R2 |
| T-129 | 3eb0bae | config не отдаёт 500 никогда (внешний try/catch, fallback на legacy) |
| T-140 | 51974e0 | Шапка: убран вложенный overflow-y-auto (sticky снова от окна, контент не перекрывается); таб-бар: Хранилище вместо «Возможностей» + якорь storage-block |
| T-138/T-141 | 0286fa7 / 51974e0 | e2e: скролл на 8 маршрутах (все ok), перекрытие шапкой ≤ 2px, переход «Хранилище» → блок в зоне видимости |
| T-142 | 5697b56 | В шапке убраны дубли навигации (навигация — только нижний таб-бар; в шапке сервисные действия) |

Проверки на 5697b56: tsc 0 · verify 0 (unit 61/61, инварианты 5/5) · e2e 13/13 (mobile-shell 8, storage-поток 2, player 3).
...
Закрыто: нижняя PWA-навигация — реализована (T-124…T-127, T-169/T-170: отдельные наборы для авторизованных и для гостей). Открытые: —, лоадер «видео готовится» вместо краша при свежей версии (ждём sourcemap-стек «j is not a function»), режим «вставить только Token value», IndexedDB-кэш.
