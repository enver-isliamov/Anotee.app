...
## 18. Safe Sharing Architecture (Review vs Invite)
**⚠️ КРИТИЧНО ДЛЯ БЕЗОПАСНОСТИ ⚠️**

Мы разделяем доступ по ссылке на два типа:

1.  **Review Link (`?projectId=X&assetId=Y`)**:
    *   Добавляет пользователя как `Guest` с полем `restrictedAssetId: Y`.
    *   Пользователь видит **только** этот файл. Доступ к другим файлам и корню проекта запрещен (API фильтрует данные).
    *   Автоматически генерируется кнопкой "Share Asset".

2.  **Invite Link (`?projectId=X&invite=true`)**:
    *   Добавляет пользователя как `Viewer` (Full Access).
    *   Автоматически снимает ограничение `restrictedAssetId`, если пользователь уже был в команде.
    *   Генерируется кнопкой "Invite Team" / "Copy Link" в шапке проекта.

3.  **Direct Link (`?projectId=X`)**:
    *   **НЕ ДОБАВЛЯЕТ** пользователя в команду.
    *   Если пользователь не участник -> 403 Forbidden.
    *   Это предотвращает доступ посторонних, просто перебирающих ID проектов.
