...
*   Конфигурация платежей хранитя в таблице `system_settings` под ключом `payment_config`.
...
## 20. System Settings (Database Keys)
**⚠️ Структура таблицы `system_settings` ⚠️**

Используется для хранения глобальной конфигурации.
...
1.  **`app_version`**:
    *   `{ "version": "v1.2602.1" }`
    *   Используется для отображения версии на главной странице.
2.  **`feature_flags`**:
    *   Хранит объект `AppConfig` из `types.ts`.
    *   Управляет лимитами Free/Pro и доступностью фич.
3.  **`payment_config`**:
    *   Хранит объект `PaymentConfig` из `types.ts`.
    *   Содержит цены, тексты тарифов и (скрыто) ключи API платежек.
