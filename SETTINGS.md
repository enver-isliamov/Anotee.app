...
## 12. Smart Polling Configuration (Performance)
**⚠️ ЛОГИКА ОПТИМИЗАЦИИ ЗАПРОСОВ ⚠️**

Для экономии лимитов Vercel Edge Requests (и денег) в приложении внедрен адаптивный механизм опроса сервера (`App.tsx` + `Player.tsx`).

*   **Active Mode (15s Interval):**
    *   Включается, когда `view.type === 'PLAYER'` **И** пользователь активен.
    *   **Триггеры активности:** Движение мыши, нажатие клавиш, клики, тачи.
    *   **Сброс таймера:** Любое действие сбрасывает таймер бездействия.

*   **Idle Mode (5m Interval):**
    *   Включается автоматически через **60 секунд** отсутствия событий ввода (даже если видео играет).
    *   Включается всегда, когда пользователь находится в Дашборде или других меню (не в Плеере).
    *   *Цель:* Если вкладка оставлена открытой на ночь, она сделает всего ~12 запросов в час вместо ~240.

*   **Force Refresh:**
    *   При отправке/удалении комментария или изменении версии происходит мгновенная ревалидация данных (`mutate`), сбрасывающая таймер.
