...
Вкладка **"Стратегия"** в `components/admin/AdminStrategyTab.tsx` имеет специфическую жесткую структуру, согласованную с бизнес-целями. Не удалять чек-листы и S.M.A.R.T. карточки.

## 21. Admin Access Control (RBAC)
**⚠️ КАК ВЫДАТЬ ПРАВА АДМИНА ⚠️**

Мы отказались от хардкода email-адресов в коде. Теперь права управляются через метаданные Clerk.

**Инструкция:**
1.  Зайдите в **Clerk Dashboard** -> **Users**.
2.  Найдите своего пользователя.
3.  Прокрутите вниз до секции **Metadata**.
4.  В поле **Public Metadata** вставьте JSON:
    ```json
    {
      "role": "superadmin"
    }
    ```
5.  Сохраните.
6.  **Перезайдите** (Log Out -> Log In) в приложении Anotee, чтобы обновить JWT токен.

*Примечание:* Также можно использовать роль `"admin"`. Бэкенд (`api/admin.js`) проверяет оба значения.
