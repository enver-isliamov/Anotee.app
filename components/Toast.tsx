...
// T-18: ошибки и предупреждения живут дольше — их нужно успеть прочитать
...
const ToastItem: React.FC<{ toast: ToastMessage; onRemove: () => void }> = ({ toast, onRemove }) => {
  ...
  const bgColors = {
    ...
    warning: 'bg-zinc-900 border-orange-900/50'
  };

  // T-18: весь тост кликабелен — на таче не нужно попадать в крошечный X
  return (
    <div onClick={onRemove} role="button" className={`pointer-events-auto flex items-center gap-3 p-3 rounded-lg border shadow-xl animate-in slide-in-from-right-full duration-300 cursor-pointer ${bgColors[toast.type]}`}>
