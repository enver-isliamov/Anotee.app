...
const STRATEGY_GOALS = [
    { id: 'Awareness', label: 'Охват (Viral)', icon: Sparkles, desc: 'Кликбейт и виральность.' },
    { id: 'Education', label: 'Обучение (Value)', icon: BookOpen, desc: 'Польза и советы.' },
    { id: 'Conversion', label: 'Продажа (Sales)', icon: TrendingUp, desc: 'Офферы и FOMO.' },
    { id: 'CaseStudy', label: 'Кейс (Trust)', icon: Briefcase, desc: 'Реальные примеры.' },
];
...
const AdminContentTabInner: React.FC = () => {
    ...
    const handleGenerateContent = async () => {
        ...
        // Auto-switch to preview tab on mobile to show loader/result
    ...
    return (
        <div className="w-full pb-24 relative">
            ...
            <div className="lg:hidden flex bg-zinc-100 dark:bg-zinc-900 p-1 rounded-xl mb-4 border border-zinc-200 dark:border-zinc-800">
                <button 
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
                ...
                <div className={`space-y-6 ${mobileTab === 'editor' ? 'block' : 'hidden lg:block'}`}>
                    
                    {/* 1. Header with Settings */}
                    <div className="bg-gradient-to-r from-violet-900/50 to-indigo-900/50 border border-indigo-500/20 p-4 md:p-6 rounded-2xl relative overflow-hidden">
                        ...
                        <div className="flex justify-between items-start relative z-10">
                            <div>
                                ...
                                <p className="text-indigo-200 text-xs leading-relaxed">
                                    Генератор контента. 
                                    <span className="opacity-70 ml-1 block md:inline">
                                        Provider: {aiConfig.provider === 'openai' ? 'GPT-4 + DALL-E' : 'Gemini + Imagen 3'}
                                    </span>
                                </p>
                            </div>
                            <button 
                            ...
                            <div className="mt-4 pt-4 border-t border-white/10 animate-in fade-in slide-in-from-top-2">
                                <div className="grid grid-cols-2 gap-4 mb-4">
                                    ...
                                    >
                                        ...
                                        <span className="text-[10px] opacity-70">Свой API Key</span>
                                    </button>
                                </div>
                                    ...
                                    <div className="flex gap-2">
                                        <div className="relative flex-1">
                                            ...
                                            <input 
                                                ...
                                                placeholder={aiConfig.hasOpenAiKey ? "Key Saved" : "sk-..."}
                    ...
                    <div className="bg-zinc-900 border border-zinc-800 rounded-2xl p-1 overflow-hidden shadow-inner group focus-within:border-indigo-500/50 transition-colors">
                        <div className="bg-zinc-950 px-4 py-2 border-b border-zinc-800 flex flex-col md:flex-row md:items-center justify-between gap-2">
                            ...
                            <button 
                                ...
                                className="flex items-center justify-center gap-1.5 px-3 py-1.5 bg-zinc-800 hover:bg-zinc-700 text-zinc-300 rounded text-[10px] font-bold transition-colors disabled:opacity-50 border border-zinc-700"
                            >
                                {isGeneratingMeta ? <Loader2 size={10} className="animate-spin" /> : <Lightbulb size={10} />}
                                {isGeneratingMeta ? 'Думаю...' : 'Сгенерировать Промпт (Free)'}
                            </button>
                        </div>
                        ...
                        <div className="bg-zinc-900 p-2 flex justify-end border-t border-zinc-800">
                            <button 
                                ...
                                className="w-full md:w-auto bg-indigo-600 hover:bg-indigo-500 text-white px-6 py-3 md:py-2 rounded-lg text-sm font-bold flex items-center justify-center gap-2 transition-all disabled:opacity-50 disabled:cursor-not-allowed shadow-lg shadow-indigo-900/20"
                            >
                ...
                <div className={`relative ${mobileTab === 'preview' ? 'block' : 'hidden lg:block'}`}>
                    <div className="lg:sticky lg:top-6">
                        <h3 className="hidden lg:flex text-xs font-bold text-zinc-500 uppercase tracking-wider mb-4 items-center gap-2">
                            <MessageSquare size={14} /> Результат
                        </h3>

                        {isLoading ? (
                            <div className="h-96 border-2 border-dashed border-zinc-800 rounded-2xl flex flex-col items-center justify-center text-zinc-600 gap-4 bg-zinc-900/30 animate-pulse">
                                <RefreshCw size={32} className="animate-spin text-indigo-500" />
                                <p className="text-xs">ИИ пишет пост...</p>
                            </div>
                        ) : result ? (
                            <div className="bg-gradient-to-b from-zinc-900 to-zinc-950 border border-zinc-800 rounded-2xl p-6 shadow-2xl relative group">
                                ...
                                <div className="pt-6 border-t border-zinc-800 flex justify-end">
                                    <button 
                                        onClick={handleCopy}
                                        className={`px-6 py-3 rounded-xl text-sm font-bold flex items-center gap-2 transition-all shadow-lg w-full md:w-auto justify-center ${
                                            copied 
                            ...
                            <div className="h-96 border border-zinc-800 rounded-2xl flex flex-col items-center justify-center text-zinc-600 gap-2 bg-zinc-900/30 p-6 text-center">
                                <Sparkles size={24} className="opacity-20" />
                                <p className="text-xs">Нажмите "Создать Контент" в редакторе.</p>
                                <button onClick={() => setMobileTab('editor')} className="lg:hidden mt-2 text-indigo-400 text-xs font-bold underline">Перейти в редактор</button>
                            </div>
                        )}
                    </div>
                </div>
