...
const VALUE_GENERATORS: Record<ContentCategory, { hooks: string[], bodies: string[], ctas: string[], images: string[] }> = {
    ...
    PHILOSOPHY: {
        hooks: [
            "Почему сейчас Lifetime, а потом подписка?",
            "Инвестируйте в инструмент, а не арендуйте его.",
            "Честная сделка с ранними пользователями.",
            "Экономика Founder's Club."
        ],
        bodies: [
            "Мы строим независимый продукт. Чтобы не зависеть от инвесторов, мы продаем вечные лицензии на старте. Это топливо для разработки.\nВ будущем, для поддержки облачной инфраструктуры, мы перейдем на стандартную подписку (SaaS). Но для вас, Founder'ов, условия останутся прежними навсегда.",
            "В 2024 году всё стало сервисом по подписке. Мы предлагаем альтернативу: заплатите один раз сейчас, пока мы в раннем доступе, и забудьте о ежемесячных списаниях в будущем. Это ваш актив.",
            "Мы не 'против' подписок — они нужны для выживания бизнеса в долгую. Но мы 'за' то, чтобы вознаградить тех, кто поверил в нас в самом начале. Founder's Club — это наша благодарность за риск.",
            "Купив лицензию сейчас, вы экономите огромную сумму в перспективе нескольких лет использования. Когда мы включим ежемесячную оплату для новых юзеров, ваш аккаунт останется безлимитным."
        ],
        ctas: [
            "Успейте забрать вечный доступ.",
            "Станьте совладельцем продукта.",
            "Зафиксируйте цену навсегда.",
            "Вступайте в клуб до перехода на SaaS."
        ],
        images: [
            "График: Цена Lifetime сейчас vs Цена подписки за 5 лет.",
            "Фото карточки Founder Member.",
            "Скриншот Roadmap с отметкой 'SaaS Launch'."
        ]
    }
};
...
export const AdminContentTab: React.FC = () => {
    ...
    const getLabel = (cat: ContentCategory | 'INTRO') => {
        switch (cat) {
            ...
            case 'PHILOSOPHY': return 'Стратегия / Оффер';
        }
    };
